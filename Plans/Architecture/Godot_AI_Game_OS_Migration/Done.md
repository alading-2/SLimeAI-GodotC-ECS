...
- Movement / Collision / Damage / Ability / Feature / AI / Attack / Projectile / Effect 第一批已迁入 `SkilmeAI.GameOS`。
...
- 旧仓库 `Src/ECS/`（289 个 C# 源文件，4.2MB）已删除，全部内容已迁移到 `SkilmeAI/GameOS/Runtime` 和 `Capabilities`。
- 旧仓库 `Data/`（227 个数据文件，1.4MB）已删除，全部内容已迁移到 `SkilmeAI/DataOS` 和 `Games/BrotatoLike/DataOS/Authoring/`。
