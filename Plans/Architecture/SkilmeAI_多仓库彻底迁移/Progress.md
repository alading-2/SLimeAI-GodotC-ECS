...
- 创建 `/home/slime/Code/SlimeAI/` 顶层工作区。
- 创建框架主仓库骨架：`/home/slime/Code/SlimeAI/SlimeAI`。
- 创建第一个游戏仓库骨架：`/home/slime/Code/SlimeAI/Games/BrotatoLike`。
- 创建 Godot 引擎位置说明：`/home/slime/Code/SlimeAI/Engine/README.md`。
- 创建工作区说明：`/home/slime/Code/SlimeAI/Workspace/README.md`。
...
- `BrotatoLike` 已通过本地项目引用接入 `SlimeAI.GameOS`。
- `Games/BrotatoLike/Tools/run-build.sh` 验证通过。
- Runtime Data / Event / Entity / Relationship / Schedule / Resource / Pool / Timer 最小内核已迁入 `SlimeAI.GameOS`。
- `Tests/SlimeAI.GameOS.Tests` 已覆盖 Event / Data / Entity / Relationship / Schedule / Pool / Timer / Resource。
