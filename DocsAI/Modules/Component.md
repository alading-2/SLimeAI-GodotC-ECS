...
- 从任意碰撞节点回溯宿主实体时，优先沿父链查 `IEntity`，不要把解析逻辑散落到业务层。
- `PickupComponent.cs` 当前整文件注释归档，不是可直接挂载的运行时组件；新增拾取功能前先恢复/重写实现并补测试。

## EntityMovementComponent 要点

- `EntityMovementComponent` 是运动策略调度器，不直接内联所有运动逻辑。
- 策略实现 `IMovementStrategy`，通过注册表按 `MoveMode` 切换。
- 策略禁止直接操作 `GlobalPosition`；位移由调度器统一执行。
- `Velocity` 表示“怎么移动”，`MovementFacingDirection` 表示“朝哪看”。
- 最终朝向优先交给 `EntityOrientationComponent`。
- 新增运动模式时：
  - 更新 `MovementEnums.cs`
  - 新增策略类
  - 注册到 `MovementStrategyRegistry`
  - 判断是否需要显式返回 facing direction
