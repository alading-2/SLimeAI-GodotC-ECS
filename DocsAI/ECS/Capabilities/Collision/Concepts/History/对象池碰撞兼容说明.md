...
- 旧方案不依赖单一属性开关时序，而是“泊车位 + 脱树 + 同步禁用 + 延迟物理同步”共同保证物理状态归零。
- 新方案不追求默认物理状态归零，而是“场外常驻 + 不切碰撞属性 + runtime state guard + 激活首帧 embargo”共同保证业务不误处理。
...
**只要一个对象靠 `Area2D` 的 entered/exited 或 monitor 回调驱动业务，而且它还要反复进出对象池，那它默认就应该走 `ParkedInTree`，并在业务碰撞入口执行 pool runtime state guard。**

原因不是它叫“子弹”还是“特效”，而是它命中了 Godot 这条最危险的组合：

- `Area2D` 事件系统
- shape / layer / monitorable 启停
- pool 复用
- 物理帧时序

新策略的关键是：默认不要再反复切 shape / layer / monitorable，也不要默认脱树；先移动到场外并停止业务处理，让旧事件和停车区事件在业务入口被 guard 丢弃。

### 7.3 推荐策略表

| 类型 | 当前结构 | 默认策略 | 结论 |
|------|----------|--------------|------|
| 玩家 Player | `CharacterBody2D`，常驻场景 | 不进池 | 通常不进对象池，不需要 pool guard |
| 敌人 Enemy | `CharacterBody2D`，带碰撞和组件子树 | `ParkedInTree` | 回池场外常驻；Hurtbox / ContactDamage 入口必须 guard；激活首帧不处理碰撞 |
| 子弹 Bullet | 你描述为 `Area2D` | `ParkedInTree` | 命中/过期回池后旧 signal 不进业务；新子弹 ready frame 后才可命中 |
| 武器 Weapon | 你描述为 `Area2D` | `ParkedInTree` | 只要存在 Area 监控和复用，就必须走 guard |
| Effect | `EffectEntity.tscn` 根节点就是 `Area2D` | `ParkedInTree` | 进入对象池复用时默认场外常驻；纯视觉则只做显隐和处理模式 |
| AbilityPreset | `Node2D` 组件容器 | 非碰撞对象 | 它不是物理实体，不是默认碰撞策略决策对象 |
...
所以从架构一致性上看，**既然你已经确认对象池 + Area 事件这条路在引擎层有物理时序风险，那同类对象就不应该继续分裂成两套离场机制。** 这次统一的机制不是“全部脱树”，而是“全部默认场外常驻 + 业务 guard + 激活首帧 embargo”。

### 7.5 性能上怎么权衡

默认 `ParkedInTree` 的成本比较对象不再是“脱树 vs 不脱树”，而是：

- parking grid 中保留物理碰撞可能产生的 pair / event / frame time 成本
- 默认脱树 / 默认关碰撞带来的 `RemoveChild/AddChild`、deferred、恢复碰撞和 pair 重建成本
- 旧 signal / 旧 timer / 旧引用进入业务导致的排查和业务错乱成本

对当前项目更合理的结论是：

- **Enemy**：回池后移动到 parking grid，ContactDamage / Hurtbox 入口 guard，避免旧 timer 或旧 signal 继续伤害。
- **Bullet / Weapon / Effect（Area2D）**：高频复用下更需要避免默认反复脱树 / 开关碰撞；激活首帧 embargo 足够覆盖旧 dispatch 风险，必要时再加 generation。

真正应该避免的是：

- 又想保留 `Area2D` 事件驱动
- 又想高频池化复用
- 还想只靠属性开关、脱树或等待一帧来替代业务 guard
...
- **敌人 `EnemyEntity`**：入池时 `ParkedInTree`，ContactDamage / Hurtbox / MovementCollision 都查 guard
- **子弹 / 武器 / Effect（只要根是 `Area2D` 且走碰撞事件）**：入池时 `ParkedInTree`，旧 signal 不进业务，新对象 ready frame 后才可命中
- **纯数据/纯组件预设（如 `AbilityPreset`）**：不是物理实体，不属于对象池碰撞策略决策对象

如果未来你觉得 parking grid 物理压力太大，那正确优化方向也不是退回默认属性开关，而是：

- 分散 parking grid
- 对特殊池显式启用 fallback detach
- 改成非 `Area2D` 事件驱动
- 用手动碰撞查询 / 命中检测
- 用更轻量的子弹管理器
...
## 八、外部依据
 ...
 | Context7 Godot 4.6 docs | `Area2D` overlap 列表每个 physics step 更新一次；`Object.set_deferred()` 在帧末赋值；`CollisionShape2D/CollisionPolygon2D.disabled` 应通过 deferred 修改 |
...
**对象池 + 物理碰撞难，根子在 Godot physics step、deferred 属性、宽相 pair 生命周期、Area signal queue 和对象复用身份叠在一起。旧方案通过“泊车位 + 脱树 + 挂树后同步禁用 + Activate 后延迟 `MoveAndSlide`”规避风险；当前默认方案改为 `ParkedInTree`：不脱树、不关碰撞、不改 layer/mask/shape，只场外停放、停业务处理，并由 pool runtime state guard 与激活首帧 embargo 保证旧 signal / 停车区事件不进入业务。**

**Detach / `disable_mode=REMOVE` 仍保留为 fallback / 对照验证，但不再是根 `CollisionObject2D` 池化对象的默认策略。**
