...
```csharp
var scene = ResourceLoading.Load<PackedScene>(ResourcePaths.Asset_Effect_020, ResourceCategory.Asset);
...
```csharp
var scene = ResourceLoading.Load<PackedScene>(ResourcePaths.Asset_Effect_004龙卷风, ResourceCategory.Asset);
EffectTool.Spawn(new EffectSpawnOptions(
    VisualScene: scene,
...
public AbilityExecutedResult Execute(CastContext context)
{
    ...
    // 1. 加载特效场景
    var effectScene = ResourceLoading.Load<PackedScene>(
