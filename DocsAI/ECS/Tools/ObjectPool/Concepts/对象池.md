...
> 当前裁决：本文保留早期对象池设计思路作追溯；涉及 Godot 物理根节点的池化对象时，以 `README.md`、`Concept.md`、`Usage.md` 和 `DocsAI/ECS/Capabilities/Collision/Concepts/对象池碰撞兼容说明.md` 为准。本文中“严禁 Reparent / 仅切 Visible + ProcessMode”的建议不适用于根节点为 `CollisionObject2D` 的池化实体。
