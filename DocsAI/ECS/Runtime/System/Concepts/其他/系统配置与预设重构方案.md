...
**1.1 创建枚举**
- `Src/ECS/Runtime/System/Core/Lifecycle/SystemGroup.cs`
- `Src/ECS/Runtime/System/Core/Lifecycle/SystemTag.cs`
- `Src/ECS/Runtime/System/Core/Lifecycle/SystemId.cs`
...
**1.3 创建资源类**
- `Src/ECS/Runtime/System/Core/Config/SystemConfig.cs`
- `Src/ECS/Runtime/System/Core/Config/SystemPreset.cs`

**1.4 创建配置服务**
- `Src/ECS/Runtime/System/Core/Config/SystemConfigService.cs`（加载和解析 SystemConfig）
- `Src/ECS/Runtime/System/Core/Config/SystemPresetService.cs`（加载和解析 SystemPreset）
...
**2.4 移除 Profile 机制**
- 删除 `Src/ECS/Runtime/System/Core/Profile/SystemProfile.cs`
- 删除 `Src/ECS/Runtime/System/Core/Profile/SystemProfileEntry.cs`
- 删除 `Src/ECS/Runtime/System/Core/Profile/SystemProfileService.cs`
...
**3.1 创建新接口文件夹**
- `Src/ECS/Runtime/System/Core/ISystem/`

**3.2 创建 ISystemFunction 接口**
- `Src/ECS/Runtime/System/Core/ISystem/ISystemFunction.cs`
...
- `Src/ECS/Runtime/System/Core/ISystem/SystemRuntimeInfo.cs`
- `Src/ECS/Runtime/System/Core/ISystem/SystemStat.cs`
...
- `Src/ECS/Capabilities/TestSystem/System/Modules/SystemTestModule.cs`
...
- `Src/ECS/Runtime/System/Core/README.md`（使用指南）
...
根据 Phase 系统深度分析报告，当前存在以下问题：

1. **Phase 配置错误导致系统无法运行**
   - 所有系统配置文件的 `AllowedAppPhases/AllowedSessionPhases/AllowedExecutionPhases` 初始值为 `0`（None）
   - 根据 `SystemRunCondition.MatchesFlags` 逻辑，`0` 表示"不限制"（通配符）
   - 但实际运行时，当前状态是 `AppPhase.Boot`（值为 1），导致所有系统 `IsStateAllowed = false`
   - **已修复**：批量修改所有系统配置为 `15`（All）

2. **Phase 设计完整但实现不完整**
   - 5 个未使用的状态值：`ShuttingDown`、`Preparing`、`Resolving`、`ModalUi`、`Cutscene`
   - 当前只实现了基础流程：启动→菜单→游戏→暂停/恢复→结束
   - 缺少：准备阶段、结算阶段、过场系统、模态窗口系统、应用关闭流程

3. **Phase 名称抽象，不符合实际场景**
   - `AppPhase.Boot` / `FrontEnd` / `InSession` 不够直观
   - `SessionPhase.Preparing` / `Resolving` 使用场景不明确
   - `OverlayPhase.ModalUi` / `Cutscene` 概念模糊

### 优化建议

#### 方案 A：简化 Phase 枚举（推荐）

**目标**：移除未使用的状态，简化为实际需要的场景
...
{
    None      = 0,
    Startup   = 1 << 0,  // 启动阶段（原 Boot）
    Menu      = 1 << 1,  // 菜单阶段（原 FrontEnd）
    Gameplay  = 1 << 2,  // 游戏阶段（原 InSession）
    
    // 预设组合
    All       = Startup | Menu | Gameplay,
}
```
...
{
    None    = 0,
    Active  = 1 << 0,  // 会话进行中（合并 Preparing/Playing/Resolving）
    Ended   = 1 << 1,  // 会话已结束
...
public enum OverlayPhase : byte
{
    None       = 0,
    PauseMenu  = 1 << 0,  // 暂停菜单
    Dialog     = 1 << 1,  // 对话框/模态窗口（原 ModalUi）
    
    // 预设组合
    All        = PauseMenu | Dialog,
}
```

**ExecutionPhase 保持不变**（已经很简洁）
...
**短期改进**：
1. 实现 `SessionPhase.Preparing` 阶段（加载资源、初始化配置）
2. 实现 `SessionPhase.Resolving` 阶段（计算分数、显示结果）
3. 为 `OverlayPhase.ModalUi` 添加通用模态窗口管理器

**中期改进**：
4. 实现 `OverlayPhase.Cutscene` 过场系统
5. 实现 `AppPhase.ShuttingDown` 应用关闭流程
...
public enum AppPhase : byte
{
    ...
    // 预设组合（新增）
    All          = Boot | FrontEnd | InSession | ShuttingDown,
    AlwaysRun    = Boot | FrontEnd | InSession,  // 常驻系统（排除关闭阶段）
    GameOnly     = InSession,                     // 仅游戏中运行
    MenuOnly     = Boot | FrontEnd,               // 仅菜单中运行
}
```

**SystemConfig 默认值改为预设**：
```csharp
// 基础系统默认配置
AllowedAppPhases = AppPhase.AlwaysRun
AllowedSessionPhases = SessionPhase.All
AllowedExecutionPhases = ExecutionPhase.All

// 战斗系统默认配置
AllowedAppPhases = AppPhase.GameOnly
...
**采用方案 C（添加 Phase 预设）+ 部分方案 B（完善实现）**

**理由**：
1. 保持设计完整性，为未来扩展预留空间
2. 通过预设简化配置，降低出错概率
3. 优先实现高价值功能（Preparing、Resolving）
4. 低优先级功能（Cutscene、ShuttingDown）暂时保留但不实现

**实施步骤**：
1. 为所有 Phase 枚举添加预设组合（`All`、`AlwaysRun`、`GameOnly` 等）
2. 更新 SystemConfig 默认值使用预设
3. 创建 `Data/Config/System/README.md` 文档说明 Phase 预设用法
4. 实现 `SessionPhase.Preparing` 和 `Resolving` 阶段（可选，Phase 7）
5. 为未实现的状态添加文档说明（预留功能）
...
### Phase 系统分析报告
- 四维状态坐标系统：AppPhase、SessionPhase、OverlayPhase、ExecutionPhase
- 5 个未使用状态：ShuttingDown、Preparing、Resolving、ModalUi、Cutscene
- 当前实现流程：启动→菜单→游戏→暂停/恢复→结束
- 设计支持流程：启动→菜单→准备→游戏→结算→暂停/恢复→过场→结束→关闭

## 实施进度

✅ 已完成

- Phase 1: 核心枚举和数据结构（SystemGroup/SystemTag、Phase 枚举、资源类、配置服务）
- Phase 2: 重构核心架构（SystemDescriptor、SystemRegistry、SystemManager、移除 Profile、SystemRunCondition）
- Phase 3: 接口重构（ISystem 文件夹、ISystemLifecycle、IProjectStateAwareSystem、SystemRuntimeInfo）
- Phase 4: 迁移现有系统（13 个系统注册代码、14 个配置文件、DefaultPreset、ResourceGenerator）
- Phase 5: 系统信息接口（SystemManager 查询接口、5 个核心系统统计信息）

⏸️ TODO（后续优化）

- Phase 6: TestSystem 重构（SystemTestModule、UI 组件、运行时操作）
- Phase 7: 文档更新（核心文档、配置文档、README、项目索引）
- Phase 8: 后续优化（自定义统计、resources_table 插件、Phase 预设）
