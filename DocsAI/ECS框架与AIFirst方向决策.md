...
当前方向应从“纯 AI 框架 / 新 GameOS 替代旧 ECS”进一步校准为：

```text
Composable Godot C# Runtime
  = 多游戏功能可组合、可裁剪、可启停的底层框架
  + ECS / Component pattern 的 Entity、Component、Data、Event、System 成熟心智模型
  + Capability / Profile 的功能组合边界
  + AI-first 的文档、入口、契约、调试、验证、工作流和观察面
```

第一目标是运行时功能解耦：不同游戏可以在同一框架下组合 Ability / Feature / Damage / AI / UI / Movement 等能力，启动前选择启用哪些系统和数据，运行中受控打开/关闭系统。AI-first 是框架的工程使用层，不是放弃 ECS 的理由，也不是比 runtime 解耦更底层的目标。
...
```text
SlimeAI 当前仓库 = Composable Godot C# Runtime + AI-first 工程层
```
...
运行时功能解耦
  用 Capability / Profile / System preset / Component composition / RuntimeCommandBuffer
  支持启动前组合、运行中受控启停和多游戏功能拼装
...
## 4.1 Component 与 System 解耦是第一目标的实现手段

运行时功能解耦必须保留 Component 和 System 两条线：

| 解耦层 | 当前含义 |
| ---- | ---- |
| Component 解耦 | GodotBridge Component 通过 C# composition profile 组合到 Entity；结构参数用 typed options 注入；跨功能状态不藏在 Component 私有字段。 |
| System 启动前组合 | 通过 Capability / GenreProfile / `system.preset` / DataOS snapshot 选择每个游戏启用的系统、数据和资源。 |
| System 运行中启停 | 通过 `SystemManager` 的 enabled/stateAllowed/blockedReason/lifecycle trace 受控启停；不是任意时刻直接拆 runtime 结构。 |
| 结构变化 | Entity spawn/destroy、Component 增删、Relationship 变化后续应走 RuntimeCommandBuffer 和 schedule phase。 |

这不是“传统 ECS 原样复刻”，而是把 ECS / Component pattern 的组合优点转译到 Godot C# + Capability owner 的框架里。

表格驱动和 AI 使用体验应排在这之后。DataOS 可以让功能组合更方便，但不能反过来让底层 Data runtime 背上所有 authoring、presentation、policy、debug 和 AI routing 职责。
