...
async function runScenesSequential(scenes, options) {
  ...
  for (const scene of scenes) {
    const manifestEntry = options.manifestEntriesByScene?.get(scene);
    const timeoutSeconds = Number.parseInt(manifestEntry?.timeoutSeconds, 10);
    const sceneOptions = Number.isFinite(timeoutSeconds) && timeoutSeconds > 0
      ? { ...runOptions, timeoutMs: timeoutSeconds * 1000 }
      : runOptions;
    const result = await runSceneWithAttempts(scene, sceneOptions);
    results.push(result);
...
async function main() {
  ...
  if (options.command === "list") {
    ...
    if (usesManifest(options)) {
      const selection = await selectManifestScenes(options);
      await printAndMaybeWrite({
        scenes: selection.scenes,
        manifest: selection.manifestSelection,
      }, options.output);
      return;
    }
  ...
  if (options.command === "run-all") {
    ...
    const selection = usesManifest(options)
      ? await selectManifestScenes(options)
      : null;
    const scenes = selection?.scenes ?? filterScenes(
      await collectScenes(DEFAULT_SCAN_ROOTS, options.includeAll),
      options.filter,
    );
    if (selection) {
      options.manifestSelection = selection.manifestSelection;
      options.manifestEntriesByScene = selection.manifestEntriesByScene;
    }
    options.logRun = await prepareLogRun(options.logDir, options.logRetentionDays);
