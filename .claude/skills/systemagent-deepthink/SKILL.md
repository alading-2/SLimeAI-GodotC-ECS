...
- 只有任务使用正式 SDD、需要跨会话恢复或需要落盘时，才读取 `Workspace/SDD/docs/SDDFormat.md`、`Workspace/SDD/docs/CLI.md` 和当前 SDD 的 `README.md`、`design/INDEX.md`、`tasks.md`、`progress.md`、`bdd.md`；如已有设计旁 FeatureSpec，也一并读取。
...
- 用户最终裁决、采用的默认假设和恢复点写入 `progress.md`；可执行事项写入 `tasks.md`；设计冻结后的功能、行为和实现指引写入 FeatureSpec；SDD `bdd.md` 只摘录本任务行为场景或 Source；参考来源和开放问题写入 `notes.md`。
