...
- 日志和截图 artifacts 保持在 `.ai-temp/scene-tests/runs`，优先读取 `index.json`、`result.json`、scene artifact、`raw/scene-log.jsonl` 和 `analysis/ai-context.md`；`combined.log` 只作为摘要和 fallback 排查入口。
