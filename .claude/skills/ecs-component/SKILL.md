---
name: ecs-component
description: 修改 SlimeAI 历史 ECS 路径下的 Component 契约、IComponent、TemplateComponent、ComponentRegistrar、GodotBridge Adapter 或新 Object/Component 模型时使用；skill ID 暂保留 ecs-component 以覆盖旧查询。
---
...
2026-06-16 后 SlimeAI 已裁决弃用 ECS 作为框架身份，正式框架名为 `SlimeAIFramework`。Component 新语义是 Godot/OOP 功能组件，不再强制是纯数据 ECS Component；允许保存内部状态。只有跨功能共享、表格驱动或需要验证追踪的状态才进入 Data。

## 必读入口

- `DocsAI/README.md`
- `SDD/project/projects/PRJ-0002-ecs-framework-refactor/design/Runtime/10.GodotOOP框架方向/README.md`
...
- Component options 不等于 Data：共享业务状态、runtime snapshot 配置、跨功能可观察结果才进入 Data；仅影响本组件 Godot bridge 策略的值留在 options。`EntityOrientationComponent.Sink` 属于 options，不新增 DataKey。
...
- Adapter / Component 可保存自己的内部状态；跨功能共享、表格驱动、需要验证追踪或持久化的状态才写入 Data / DataKey。
