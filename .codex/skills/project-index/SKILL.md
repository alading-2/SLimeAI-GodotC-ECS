...
- **系统配置 DataNew** → `Data/DataNew/System/SystemConfigData.cs`（优先数据源）
- **系统预设 DataNew** → `Data/DataNew/System/SystemPresetData.cs`（优先数据源；默认预设显式加载 TestSystem / MouseSelectionSystem，不整体启用 Debug / Test 标签）
- **系统配置资源回退** → `Data/Config/System/System/SystemConfig.cs` / `Data/Config/System/System/Resource/*.tres`
- **系统预设资源回退** → `Data/Config/System/Preset/SystemPreset.cs` / `Data/Config/System/Preset/Resource/*.tres`
- **项目状态服务** → `Src/ECS/Base/System/Core/State/ProjectStateService.cs`
- **项目状态事件协议** → `Data/EventType/Global/GameEventType_Global_ProjectState.cs`
...
- **生命周期接口** → `Src/ECS/Base/System/Core/Lifecycle/ISystem.cs` / `Src/ECS/Base/System/Core/Lifecycle/ISystemLifecycle.cs` / `Src/ECS/Base/System/Core/Lifecycle/IProjectStateAwareSystem.cs`
- **关键边界**：代码注册只管 `SystemId + Factory`，`SystemConfig` 管系统元数据和三域运行条件，`SystemPreset` 管启动装载选择，`Enable/Disable` 管运行时人工开关，`OnStarted/OnStopped` 管实际运行态
...
- **技能配置** → `Data/Data/Ability/AbilityConfig.cs`（`FeatureGroupId` 仅用于 UI / 调试分组；`FeatureHandlerId` 是运行时处理器映射主键）
