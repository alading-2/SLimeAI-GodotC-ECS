...
- **系统运行时管理器** → `Src/ECS/Base/System/Core/SystemManager.cs`
- **系统注册表** → `Src/ECS/Base/System/Core/SystemRegistry.cs`（重复 `SystemId` 记录错误日志并保留首个描述符）
- **系统描述符** → `Src/ECS/Base/System/Core/SystemDescriptor.cs`
- **系统 Profile** → `Src/ECS/Base/System/Core/Profile/SystemProfile.cs` / `Data/Config/System/DefaultSystemProfile.tres`
- **Profile 条目** → `Src/ECS/Base/System/Core/Profile/SystemProfileEntry.cs`
...
- **关键边界（V2）**：`Add/Remove` 管实例存在性，`Enable/Disable` 管显式人工开关，`OnStarted/OnStopped` 管实际运行态，`SystemProfile` 管项目启动装配表与显式 `SystemId` 覆盖
