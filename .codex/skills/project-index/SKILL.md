...
- **启动桥接** → `Src/Autoload/AutoLoad.cs`
- **系统运行时管理器** → `Src/ECS/Base/System/Core/Godot/SystemManager.cs`
- **系统注册表** → `Src/ECS/Base/System/Core/Runtime/SystemRegistry.cs`
- **系统描述符** → `Src/ECS/Base/System/Core/Runtime/SystemDescriptor.cs`
- **项目状态服务** → `Src/ECS/Base/System/Core/Runtime/ProjectStateService.cs`
- **运行条件** → `Src/ECS/Base/System/Core/Runtime/SystemRunCondition.cs`
...
- **运动通用工具** → `Src/ECS/Base/System/Movement/MovementHelper.cs`（跨策略通用能力）
...
- **AutoLoad（全局单例管理）** → `Src/Autoload/AutoLoad.cs`（当前只做启动桥接与根服务承载，新系统优先走 `SystemRegistry + SystemManager`）
