...
技能流水线：`TryTrigger → CanUse检查 → SelectTargets → ConsumeCharge → StartCooldown → ConsumeCost → FeatureSystem.OnFeatureActivated → IFeatureHandler.OnActivated → IFeatureHandler.OnExecute → Ability.Executed → FeatureSystem.OnFeatureEnded`
