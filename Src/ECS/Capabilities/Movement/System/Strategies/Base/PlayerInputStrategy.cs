...
/// <para>所需 Data（实体属性，非 MovementParams）：<c>GeneratedDataKey.MoveSpeed</c>（最大速度），<c>GeneratedDataKey.Acceleration</c>（平滑系数）。</para>
...
public class PlayerInputStrategy : IMovementStrategy
{
    ...
    public MovementUpdateResult Update(IEntity entity, Data data, float delta, in MovementParams @params)
    {
        ...
        float speed = data.Get<float>(GeneratedDataKey.FinalMoveSpeed); // 最终移动速度
        float acceleration = data.Get<float>(GeneratedDataKey.Acceleration); // 速度插値系数，越大响应越快

        Vector2 inputDir = InputManager.GetMoveInput(); // 输入系统给出的移动方向
        if (inputDir.LengthSquared() > 0.001f)
        {
            data.Set(GeneratedDataKey.LastMoveDirection, inputDir.Normalized()); // 记录最后一次主动移动方向，供 Dash 等技能在静止时复用
        }

        Vector2 targetVelocity = inputDir.Normalized() * speed;
        Vector2 currentVelocity = data.Get<Vector2>(GeneratedDataKey.Velocity); // 上一帧基础速度，用于平滑过渡

        // Lerp 平滑加速（指数衰减公式，帧率无关）
        Vector2 newVelocity = currentVelocity.Lerp(targetVelocity, 1.0f - Mathf.Exp(-acceleration * delta));

        data.Set(GeneratedDataKey.Velocity, newVelocity);
