...
**设计理念**：详见 [`Docs/框架/ECS/Entity/Entity架构设计理念.md`](../../../../../Docs/框架/ECS/Entity/Entity架构设计理念.md)
...
| 文件 | 职责 |
|------|------|
| [EntityManager.cs](EntityManager.cs) | 核心生命周期管理（Spawn, Register, Destroy）、基础查询（GetEntitiesByType, GetEntityById）、全局查询（GetAllEntities, GetEntitiesByInterface） |
| [EntityManager_Component.cs](EntityManager_Component.cs) | Component 管理（RegisterComponents, AddComponent, GetComponent, RemoveComponent） |
| [EntityManager_Migration.cs](EntityManager_Migration.cs) | Entity 迁移扩展（新建目标实体、迁移受控 Data、销毁源实体） |
| [EntityManager_Ability.cs](../../System/AbilitySystem/EntityManager_Ability.cs) | Ability 管理（AddAbility, RemoveAbility, GetAbilities） |
...
- **数据源**: `EntitySpawnConfig.Config` (通常来自 `Data/DataNew` POCO 或字典配置；旧 `.tres` 不再作为运行时主入口)
...
> 仅在维护 EntityManager 内部生命周期流程时需要关注；业务层不要绕过 EntityManager 调用 `QueueFree`。
...
- **架构设计**：[`Docs/框架/ECS/Entity/Entity架构设计理念.md`](../../../../../Docs/框架/ECS/Entity/Entity架构设计理念.md)
- **详细设计**：[`Docs/框架/ECS/Entity/EntityManager设计说明.md`](../../../../../Docs/框架/ECS/Entity/EntityManager设计说明.md)
