...
日期目录示例：`Docs/测试/场景测试日志/runs/2026-05-01/21-30-00/`。如果同一秒内创建多个运行目录，会自动追加序号：`21-30-00-002`。
...
- 删除目标从返回 JSON 的 `logIndex` 得到，例如 `Docs/测试/场景测试日志/runs/2026-05-02/08-56-30/index.json` 对应删除 `Docs/测试/场景测试日志/runs/2026-05-02/08-56-30`。
...
```bash
rg -n -m 120 -C 2 "\\[MovementCollisionRuntimeTest\\]|\\[MovementCollisionPolicy\\]" Docs/测试/场景测试日志/runs/<日期>/<时间>/**/combined.log
```

只看某个场景目录：

```bash
rg -n -m 80 -C 3 "ERROR:|\\[FAIL\\]" Docs/测试/场景测试日志/runs/<日期>/<时间>/001_*/combined.log
```

如果某个上下文仍然太多，继续叠加关键词：

```bash
rg -n -m 80 -C 3 "\\[MovementCollisionRuntimeTest\\].*owner|TeamFilter|友军|敌方" Docs/测试/场景测试日志/runs/<日期>/<时间>/**/combined.log
```
...
需要截图 Linux 桌面、游戏运行画面、UI 或报错窗口时，先打开有界面 Godot 场景：
...
执行后 3 秒内切到目标窗口并框选截图。截图是视觉验证流程，不替代 CLI 日志测试；两者结论要分开记录。

## 8. 当前没实现什么

- 不打开 Godot 编辑器窗口；这是 headless CLI 测试，不是人工视觉调试。
- 不做截图或像素级视觉断言；视觉表现仍需要专门测试或人工打开 Godot。
...
```bash
git status --short
node .codex/skills/godot-scene-test/scripts/godot-scene-runner.mjs run-many <相关场景1> <相关场景2> --build --continue-on-fail --attempts 2 --errors-only --log-dir Docs/测试/场景测试日志/runs
rg -n -m 80 -C 3 "ERROR:|\\[ERROR\\]|\\[FAIL\\]|Exception|Cannot instantiate" Docs/测试/场景测试日志/runs/<日期>/<时间>
rm -r -- Docs/测试/场景测试日志/runs/<日期>/<时间>
git status --short
```
...
```bash
node .codex/skills/godot-scene-test/scripts/godot-scene-runner.mjs run-all --build --continue-on-fail --errors-only --log-dir Docs/测试/场景测试日志/runs
```
