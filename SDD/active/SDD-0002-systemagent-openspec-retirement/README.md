...
- **Current Task**: done
- **Last Conclusion**: Workspace/SystemAgent 默认入口、workflow、role、policy、gate、protocol 已切换为 SDD-first；OpenSpec 专属协议文件已删除；.ai-config 源已更新并同步；验证通过。
- **Next Action**: 如需收尾，可在用户确认后将本 SDD 移入 done 并提交根仓改动；push 仍需显式确认。
- **Open Blockers**: none
