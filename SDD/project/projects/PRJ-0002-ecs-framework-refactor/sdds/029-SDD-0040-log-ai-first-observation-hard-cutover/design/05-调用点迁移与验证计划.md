...
> 更新：2026-06-11
> 状态：historical design note；迁移切片仍可参考，analyzer 默认目录以项目级第二部分 `03-最终设计与完成清单.md` 为准
...
- `logctl analyze` 输出 `summary.md`、`ai-context.md`、`flows/flows.jsonl`、`noise/templates.jsonl`、`failures/`、`noise/`、`missing-fields/` 和 `raw/entries.jsonl`；默认不输出 `by-owner` / `by-phase` raw 复制分桶。
