...
- **目标**: 建立 Data 完整重构第一切片。
- **测试优先**: 先写 Catalog RED tests。
- **实现范围**: `DataValueType`、policy enum、`DataDefinition`、`DataDefinitionCatalog`、`BuildCatalog`、`DataComputeRegistry` 骨架、`LegacyDataAuditReport`。
- **审计边界**: `LegacyDataAuditReport` 只做迁移报告，禁止成为运行时 fallback。

## 明确不做

- **不做 records apply**: 属于 SDD-0017。
- **不做 Entity bootstrap**: 属于 SDD-0017。
- **不做 Feature modifier bridge**: 属于 SDD-0015。
- **不删除旧路径**: 属于 SDD-0019。

## 任务提示

1. 建立 RED/GREEN/REFACTOR 记录格式。
2. 写 Catalog RED tests：duplicate stable key、empty key、unknown value type、default conversion failure、invalid policy、unknown dependency、compute cycle、missing resolver。
3. 实现最小 `DataDefinitionCatalog`。
4. 实现 descriptors fixture 的 `BuildCatalog`。
5. 实现 `DataComputeRegistry` 骨架和 resolver 绑定校验。
6. 实现 `LegacyDataAuditReport`。
7. 更新 SDD 任务和进度。
8. 运行目标 Data 小测试、必要构建和 `python3 Workspace/SDD/sdd.py validate SDD-0012`。
