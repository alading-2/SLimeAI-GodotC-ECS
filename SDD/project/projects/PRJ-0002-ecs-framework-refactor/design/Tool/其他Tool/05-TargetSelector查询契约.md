...
> 裁决：TargetSelector 是剩余 Tools 中最需要优先完善的功能之一。执行时应升级为 `TargetQueryEngine` / `TargetQueryResult` 语义：只保目标查询功能，不保静态全局扫描和 list-only 旧 API 兼容。
...
执行型 SDD 建议直接引入 `TargetQueryEngine` / `TargetQueryResult` 作为 current 入口。`EntityTargetSelector.Query(query)` 如果为了迁移暂时保留，只能作为临时 facade，不能作为 DocsAI current 教给 AI 的入口；切片完成前必须决定删除、internal 化或明确 test-only。
...
- 增加 `TargetQueryResult` / diagnostics；list-only facade 只允许作为迁移桥，不进入 current 文档入口。
...
- 默认 source 接 EntityRegistry 或显式 source；NodeLifecycle source 仅作为临时或 internal fallback。
...
- 不建议长期保留 `EntityTargetSelector.Query(query)` 作为 AI current 入口。
