...
> 更新：2026-06-04
> 状态：planning input, user review calibrated, 2026-06-07 final confirmation
> 裁决：本设计包不直接执行实现。后续执行型 SDD 按功能切片 hard cutover：只保功能，不保旧 API 兼容；优先 RuntimeMountRegistry 与 TargetQueryEngine。用户已确认默认 `/root/SlimeAIRuntime`、资源 strict fail-fast、Common Utilities 放 `Src/ECS/Tools/CommonUtilities/`、NodeLifecycle 迁 Runtime、TargetSelector 不做兼容桥。2026-06-07 ResourceManagement 校准：`res://` 不是问题，路径移动由 `resource-path-migration` workflow + generator + diagnostics 闭环。
...
| T3 | ResourceLoading / ResourceManagement + CommonTool 收口 | strict lookup、删除 contains fallback、LoadPath source policy、structured result、catalog diagnostics；迁移 `CommonTool.LoadPackedScene` 调用点并删除或 internal 化当前 `CommonTool` owner；接入 `resource-path-migration` workflow，资源移动后自动替换旧引用并检查残留。 | `resource-loading-hard-cutover` |
...
- 移动/重命名资源后使用 `resource-path-migration` skill 替换 old/new path，运行 ResourceGenerator，并用 `rg` 检查旧路径残留。
