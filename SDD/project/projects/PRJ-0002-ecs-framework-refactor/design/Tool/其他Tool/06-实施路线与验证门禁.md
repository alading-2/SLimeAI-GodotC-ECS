...
> 更新：2026-06-04
> 状态：planning input, user review calibrated
> 裁决：本设计包不直接执行实现。后续执行型 SDD 按功能切片 hard cutover：只保功能，不保旧 API 兼容；优先 RuntimeMountRegistry 与 TargetQueryEngine。用户已确认默认 `/root/SlimeAIRuntime` 和资源 strict fail-fast；Common Utilities 概念保留但必须受约束。
...
| T2 | TargetQueryEngine / TargetSelector 功能 | query validation、ResolveOrigin/Forward、candidate source、diagnostics、deterministic RNG、safe sorting、Single explicit target；迁移 Ability/AI/Feature 调用点，切片结束前不保长期 list-only facade 作为 AI current 入口。 | `target-query-engine-hard-cutover` |
| T3 | ResourceLoading / ResourceManagement + CommonTool 收口 | strict lookup、删除 contains fallback、LoadPath source policy、structured result、catalog diagnostics；迁移 `CommonTool.LoadPackedScene` 调用点并删除或 internal 化当前 `CommonTool` owner。 | `resource-loading-hard-cutover` |
...
| T5 | Common Utilities | 保留通用工具区域，但不保留 `CommonTool.SomeHelper()` 杂物箱；建立位置、manifest、禁止项和测试规则。 | 可并入 T3，或独立 `common-utilities-contract` |
...
- `EntityTargetSelector.Query(query)` 可以作为执行期临时 facade，但 DocsAI current 入口应转向 `TargetQueryEngine` / diagnostics result。
...
- 推荐先确认目录：`Src/ECS/Common/Utilities/` 或 `Src/ECS/Runtime/Common/`。
...
说明：执行型 SDD 可按切片定义例外，但例外必须是 test-only、migration note 或 internal diagnostics，不得作为 AI current 使用入口。

执行前仍需用户确认：

- Common Utilities 的最终目录位置。
- NodeLifecycle 是否迁到 `Src/ECS/Runtime/NodeLifecycle/`。
