...
> 更新：2026-06-04
> 状态：planning input
> 裁决：本设计包不直接执行实现。后续执行型 SDD 按功能切片 hard cutover：只保功能，不保旧 API 兼容；优先 RuntimeMountRegistry 与 TargetQueryEngine。
...
| T3 | ResourceLoading / ResourceManagement + CommonTool 删除 | strict lookup、删除 contains fallback、LoadPath source policy、structured result、catalog diagnostics；迁移 `CommonTool.LoadPackedScene` 调用点并删除 `CommonTool` owner。 | `resource-loading-hard-cutover` |
| T4 | MathFormula / deterministic random | 拆 `MyMath` owner，概率/采样可注入 seed/RNG，公式 owner 写入 Capability 文档，Geometry2D 仍为纯几何。 | `math-formula-random-hard-cutover` |
| T5 | NodeLifecycleRegistry | 低层 Node registry、owner/source metadata、snapshot diagnostics、invalid cleanup；业务路径删除 `GetAllNodes` / `GetNodesByInterface` 全局扫描依赖。 | 可并入 T1/T2，或独立 `node-lifecycle-registry-cutover` |
...
- `CommonTool` 是删除目标，不新增方法，不保长期 facade。
