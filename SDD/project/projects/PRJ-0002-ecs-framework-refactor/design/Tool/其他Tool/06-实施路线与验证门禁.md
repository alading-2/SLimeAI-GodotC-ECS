...
> 裁决：本设计包不直接执行实现。后续执行型 SDD 按功能切片 hard cutover：只保功能，不保旧 API 兼容；优先 RuntimeMountRegistry 与 TargetQueryEngine。用户已确认默认 `/root/SlimeAIRuntime`、资源 strict fail-fast、Common Utilities 放 `Src/ECS/Tools/CommonUtilities/`、NodeLifecycle 迁 Runtime、TargetSelector 不做兼容桥。2026-06-07 ResourceManagement 校准：`res://` 不是问题，路径移动由 project directory / `project-filesystem` workflow + generator + diagnostics 闭环；`ResourceManagement` 不作为长期“管理器”概念保留，执行期收敛为极薄 `ResourceLoading` 统一加载工具。
...
- 移动/重命名目录或资源后使用 project directory / `project-filesystem` skill 替换 old/new path，运行 ResourceGenerator，并用 `rg` 检查旧路径残留。
...
Scenario: Project directory migration verifies old path residue
  ...
  When project directory / project-filesystem runs with apply mode
