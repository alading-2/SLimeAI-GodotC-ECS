...
> 裁决：`CommonTool.LoadPackedScene` 不应继续留在杂项入口，应迁入 `ResourceManagement` / `ResourceLoading`；但用户确认“通用工具区域”概念可保留。执行型 SDD 应删除或 internal 化当前 `CommonTool` 类，不保留无约束杂物箱；如需 Common Utilities，应另设受约束位置、manifest、禁止项和测试。
...
- 若新增 Common Utilities，建议从 `Tools/` 独立出来，例如 `Src/ECS/Common/Utilities/` 或 `Src/ECS/Runtime/Common/`。
