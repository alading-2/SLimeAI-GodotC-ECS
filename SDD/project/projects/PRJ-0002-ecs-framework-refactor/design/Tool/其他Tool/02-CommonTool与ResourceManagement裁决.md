...
> 裁决：`CommonTool` 不应继续作为杂项扩展点，也不应作为长期兼容 facade；执行型 SDD 中应迁移 `LoadPackedScene` 调用点并删除 `CommonTool` owner。资源路径加载需求回归 `ResourceManagement` / `ResourceLoading` 的 AI-first 加载契约。
...
执行阶段不应长期保留现有方法。它可以在切片中短暂存在用于编译迁移，但完成前必须删除或 internal 化：

- 不新增新方法。
- 不作为 DocsAI/ECS/Tools 的正式 owner。
- 调用点迁移到更明确的 `ResourceManagement.LoadPackedScenePath(...)` 或 `ResourceLoading.LoadPackedScene(...)`。
