...
- `logctl analyze` 输出 `summary.md`、`ai-context.md`、`flows/flows.jsonl`、`noise/templates.jsonl`、`failures/`、`missing-fields/` 和 `raw/entries.jsonl`；默认不输出 `by-owner` / `by-phase` raw 复制分桶。
...
- Given 用户只想看某个 owner 的语义结论，When 执行 `logctl query --analysis-dir <run>/analysis owner=Ability`，Then 输出 flow conclusion 或 success template，不要求重新运行游戏。
- Given 用户只想按 `sourceFile` 等原始字段下钻，When 执行 `logctl query --file <run>/analysis/raw/entries.jsonl sourceFile=...`，Then 输出 raw entry 证据，并保持这是显式 raw 下钻。
