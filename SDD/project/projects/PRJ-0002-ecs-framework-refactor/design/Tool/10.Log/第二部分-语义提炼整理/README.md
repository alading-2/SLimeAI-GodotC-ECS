...
> 更新：2026-06-11
> 状态：current contract（analyzer 默认入口已落地；不代表源码调用点迁移完成）
> 入口：本文件
...
第二部分只改一件事：**把离线整理的单位从「一条日志」换成「一次操作(flow 结论)」**，让产物以结论为主、行数显著变少、失败一眼可见。

第二部分不负责证明 live stdout 已经收口，也不负责把 `Src/ECS` 全部 `_log.Info` / `_log.Debug` / 测试打印迁移成 flow。这个大任务单独进入 `../第三部分-源码调用点语义化/README.md`。
...
6. `../第三部分-源码调用点语义化/README.md` —— live 打印仍分离的真实原因和后续源码调用点迁移方向。
...
- `Src/ECS` 源码调用点语义化未完成：当前仍有大量普通 `_log.*` 调用和少量直接打印；第二部分的 analyzer DONE 不能作为“Log 全部改完”的证据。
