...
```text
SDD/project/projects/PRJ-0002-ecs-framework-refactor/design/Tool/10.Log/source-request.md
```
...
- **整理链路不够**：`raw/scene-log.jsonl` 不是 AI 分析入口；必须先生成摘要、分桶、flow、失败、噪声和缺字段报告。
...
raw/scene-log.jsonl
  -> logctl analyze
  -> analysis/summary.md + ai-context.md + by-owner + by-phase + flows + failures + noise + missing-fields
...
| analyzer 产物太薄 | 当前 `ai-context.md` 只列 run metadata、owner 和 query 示例；没有 top noise、missing field、flow digest、owner 建议 | `logctl analyze` 必须生成面向 AI 的 digest，而不是只拆 raw JSONL。 |
| flow 边界错误 | 当前 `flows/flows.json` 把几乎所有有 `operation` 的 entry 都纳入 flow | flow 必须由 `channel=Flow`、`entryType=flow_summary/flow_step` 或明确 OperationTrace 标记决定。 |
...
| JSONL 仍然太大 | `logctl analyze` 必须输出 markdown digest 和分桶目录，AI 默认不读 raw。 |
...
- runner / logctl 产物中存在 `raw/`、`by-phase/`、`by-owner/`、`flows/`、`failures/`、`noise/`、`missing-fields/`、`summary.md`、`ai-context.md` 等分层目录。
...
- `Workspace/Tools/logctl/logctl.mjs analyze` 已能拆出 `by-owner` / `by-phase` / `noise` / `missing-fields`，但缺 `summary.md`、markdown noise/missing-fields/flow index，`ai-context.md` 信息量不足。
