...
1. 扫描 `EmitDynamic|OnDynamic|Action<object>` 调用点，确认只有 Feature/Trigger 相关。
2. 新增 typed Feature event action registry。
3. 迁移 `EmitEventAction` 为 typed action 或删除未使用路径。
4. 新增 Ability trigger binding registry，替代 `AbilityTriggerEvent` 字符串动态订阅。
5. 删除 EventBus dynamic API 和 object branch。
6. 更新 `DocsAI/ECS/Runtime/Event/Event系统说明.md`、Feature/Ability 文档和 `ecs-event` skill。
