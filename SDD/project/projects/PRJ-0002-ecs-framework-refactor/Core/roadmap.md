...
2026-06-16 override：用户已裁决弃用 ECS 作为框架身份，正式框架名为 `SlimeAIFramework`。旧 `Data Runtime Simplification -> Data Type Contract -> Generated RuntimeId Storage` 路线暂停；用户已删除 SDD-0044，项目不再登记为 pending/current。当前已新增 `Runtime/10.GodotOOP框架方向/`，冻结 Object / Component / System / Feature / Event / Data 概念；Data 名字保留，后续按 DataBinding、受控共享状态、表格驱动和 DataModifier 收窄重新设计。历史 Data、Entity、System、Tools 等 SDD 完成记录仍保留为现状证据，但不再证明框架必须继续走 ECS/Data 核心方向。
...
| `design/Runtime/2.Data系统优化/4.Data验证与Registry简化/01-DataComputeRegistry单例与Catalog验证收敛.md` | superseded | — | 用户已删除 SDD-0044；不再执行 registry singleton 小修，只保留 fatal 前 structured report 思想 |
...
| `design/Runtime/10.GodotOOP框架方向/` | current | — | SlimeAIFramework 当前方向设计包：框架名、Object/Component/System/Feature/Event/Data 边界、DataBinding、DataModifier 和迁移验证路线 |
...
| P0 | `design/Runtime/10.GodotOOP框架方向/Data/` | **Next**：创建 Data 进入条件与 DataBinding 设计 SDD，先定义 authority、Component 镜像、对象池同步和 modifier 范围 |
