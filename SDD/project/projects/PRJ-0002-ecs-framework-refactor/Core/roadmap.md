...
| `design/Tool/其他Tool/` | proposed | TBD | 2026-06-07 consolidated：current 事实源收敛为 `README.md` + `01~06`；Common Utilities 放 `Src/ECS/Tools/CommonUtilities/`、NodeLifecycle 迁 Runtime、TargetSelector 不做兼容桥；ResourceManagement 用户校准为不保留管理器概念，只保留极薄 ResourceLoading facade + generated catalog + diagnostics，`res://` 不是问题，目录/资源移动用 project directory / `resource-path-migration` workflow；后续建议按 RuntimeMountRegistry、TargetQueryEngine、ResourceLoading、NodeLifecycleRegistry、Common Utilities、MathFormula hard cutover 拆执行型 SDD |
