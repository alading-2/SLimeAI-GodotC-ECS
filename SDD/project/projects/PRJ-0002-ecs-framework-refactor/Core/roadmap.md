...
| `design/Tool/10.Log/` | blocked | SDD-0040 | 结构化 Logger、C# stdout summary / buffered JSONL file sink、Validation helper、runner wrapper 已落地；2026-06-10 样本复查指出 analyzer digest、flow 边界、owner 字段契约和 Validation artifact 状态区分仍需补强；Godot scene smoke blocked 于当前无有效承载游戏 runner |
...
| P0 | `design/Tool/10.Log/README.md` + `design/Tool/10.Log/07-当前样本日志问题与整理方案.md` | **SDD-0040 blocked + follow-up**：结构化 `LogEntry`、severity/outcome、`OperationTrace`、C# stdout summary + buffered JSONL file、optional Godot editor sink、`logctl`、ValidationSession 和 runner wrapper 已落地；后续按样本复查补 analyzer markdown digest、flow 识别、owner 字段契约和 Validation artifact 状态；Godot scene smoke blocked 于当前无有效承载游戏 runner |
