...
| `design/Tool/10.Log/` | pending | SDD-0040 | Log AI-first Observation Hard Cutover 已创建执行胶囊；同批处理 Logger core、C# stdout summary / buffered JSONL file sink、Validation helper、runner analyzer、业务 flow 和 owner Log 文档 |
...
| P0 | `design/Tool/10.Log/README.md` + `sdds/029-SDD-0040-log-ai-first-observation-hard-cutover/execution-prompt.md` | **SDD-0040 pending**：Log AI-first Observation Hard Cutover；结构化 `LogEntry`、severity/outcome、`OperationTrace`、C# stdout summary + buffered JSONL file、optional Godot editor sink、`Config/Log` profile、`logctl`、ValidationSession、runner analyzer、owner `Log.md` 和降噪回写同批执行；不建议只做 Logger 热路径小修，也不建议把详细日志简单改成每条 `Console.WriteLine` |
