...
| `design/Tool/其他Tool/` | proposed | TBD | 2026-06-04 user review 已完成：功能优先、代码可丢弃；2026-06-07 用户最终确认 Common Utilities 放 `Src/ECS/Tools/CommonUtilities/`、NodeLifecycle 迁 Runtime、TargetSelector 不做兼容桥；ResourceManagement 用户校准为 ResourceLoading facade + generated catalog + diagnostics，`res://` 不是问题，路径移动用 `resource-path-migration` skill；后续建议按 RuntimeMountRegistry、TargetQueryEngine、ResourceLoading、NodeLifecycleRegistry、Common Utilities、MathFormula hard cutover 拆执行型 SDD |
...
| P0 | `design/Tool/其他Tool/10-2026-06-07-用户最终确认与执行口径.md` + `design/Tool/其他Tool/05-TargetSelector查询契约.md` | **建议新建 SDD**：Target Query Engine Hard Cutover；补 query validation、resolved origin/forward、candidate source、diagnostics、deterministic RNG、safe sorting，一次性迁移 Ability/AI/Feature 调用点并删除旧 `EntityTargetSelector.Query` list-only API |
| P1 | `design/Tool/其他Tool/09-2026-06-07-ResourceManagement深度分析.md` + `design/Tool/其他Tool/11-2026-06-07-ResourcePathMigrationSkill设计.md` + `design/Tool/其他Tool/10-2026-06-07-用户最终确认与执行口径.md` + `design/Tool/其他Tool/02-CommonTool与ResourceManagement裁决.md` | **建议新建 SDD**：Resource Loading Hard Cutover；将 ResourceManagement 简化为统一 ResourceLoading facade，迁出 `CommonTool.LoadPackedScene`，删除 contains fallback，补 LoadPath source policy、structured result、ResourceCatalogDiagnostics、资源变更生成器 gate，并接入 `resource-path-migration` workflow；Common Utilities 固定放 `Src/ECS/Tools/CommonUtilities/` |
