...
| `design/Tool/10.Log/` | blocked | SDD-0040 | T1 结构化 Logger、C# stdout summary / buffered JSONL file sink、Validation helper、runner wrapper 已落地；T2 analyzer digest、flow 边界、semantic missing-fields、owner hot-spot cleanup 和 Validation artifact 状态区分仍需补强；Godot scene smoke blocked 于当前无有效承载游戏 runner |
...
| P0 | `design/Tool/10.Log/README.md` + `design/Tool/10.Log/07-当前样本日志问题与整理方案.md` | **SDD-0040 blocked + T2 follow-up**：结构化 `LogEntry`、severity/outcome、`OperationTrace`、C# stdout summary + buffered JSONL file、optional Godot editor sink、`logctl`、ValidationSession 和 runner wrapper 已完成 T1；后续必须按当前样本补 analyzer markdown digest、gate 状态、flow 边界、semantic missing-fields、owner hot-spot cleanup 和 Validation artifact 状态；Godot scene smoke blocked 于当前无有效承载游戏 runner |
