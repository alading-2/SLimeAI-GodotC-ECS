...
| `design/Runtime/2.Data系统优化/5.Data类型系统重构/` | pending | TBD | 2026-06-15 新根本裁决：功能解耦优先，数据形态统一不是目标；后续拆 Data Runtime Simplification、Data Type Contract、Generated RuntimeId Storage，SDD-0044 并入或降为前置子任务 |
...
| P0 | `design/Runtime/2.Data系统优化/5.Data类型系统重构/09-Data系统根本裁决与重构路线.md` | **Next**：创建 Data Runtime Simplification Hard Cutover，瘦身 runtime descriptor / definition / policy / presentation 边界，并写清 Data 进入条件 |
| P0 | `design/Runtime/2.Data系统优化/5.Data类型系统重构/09-Data系统根本裁决与重构路线.md` | **Next**：创建 Data Type Contract Hard Cutover，统一 `DataTypeContract` / `DataValueCodec` / typed default / fixed slot type / computed output type validation；SDD-0044 并入或作为前置子任务 |
| P1 | `design/Runtime/2.Data系统优化/5.Data类型系统重构/09-Data系统根本裁决与重构路线.md` | **Later**：Generated RuntimeId Storage，让 `DataKey<T>` 携带 stable key + runtime id，storage 优先数组索引；profiler 证明后再做 numeric lane |
| P1 | `design/Runtime/2.Data系统优化/5.Data类型系统重构/09-Data系统根本裁决与重构路线.md` | **Conditional**：完整传统 ECS 分支评估，只在 Data 简化和 runtimeId 后仍证明 Data 是主瓶颈时启动 |
| P0 | `design/Runtime/2.Data系统优化/4.Data验证与Registry简化/01-DataComputeRegistry单例与Catalog验证收敛.md` | **SDD-0044 pending / scope needs update**：`DataComputeRegistry.Default` 和 catalog report 方向仍成立，但不应孤立优先执行，应并入 Data Type Contract hard cutover |
