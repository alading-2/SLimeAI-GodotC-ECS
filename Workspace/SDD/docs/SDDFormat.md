...
| `bdd.md` | 本任务行为场景摘录、设计旁 BDD 引用或不适用说明。 |
...
BDD 是行为例子，不是每个 SDD 必填的形式文档。

规则：

1. 长期行为场景优先靠近项目级设计文档，或作为设计文档的“行为验收”小节。
2. 子 SDD `bdd.md` 可以只摘录本任务实际执行的场景，并用 `Source:` 链接回设计源。
3. 如果设计旁 BDD 已经完整，子 SDD 可以只列 `Executed scenarios`，不复制全文。
4. 文档治理、目录整理、skill/rule 调整、研究任务默认可以 `Required: false`，说明原因即可。
5. validate 不应要求每个 SDD 都有多条 Scenario；有有效 Source 引用或 not-required reason 即可。
