...
python3 Workspace/SystemAgent/Tools/session-adapter/session_adapter.py stale-report --source-root /home/slime/.codex/sessions/2026/06/10
```
...
`index.json` 会升级到 `schema_version=4`。digest entry 包含：
...
- 命令分类：`command_category_counts`，类别包括 `read`、`edit`、`sdd_state_write`、`validation`、`git_inspection`、`git_write`、`external_probe`、`unknown`
- 恢复信号：`meaningful_user_turns`、`files_mentioned`、`validation_signals`、`interrupted`、`final_conclusion`
- 效率信号：`verification_loops` 只从 `edit` 或 `sdd_state_write` 后的 `validation` 计算；`git status/diff/log` 只算 `git_inspection`
...
- `retry_count`：同一命令后续重试次数
- `recovered`：`yes/no/unknown`
- `final_impact`：`blocked`、`worked_around`、`not_relevant`、`unknown`

## Coverage / Stale Report

`stale-report` 比较 Codex source JSONL 与 ChatHistory `index.json`：

- `coverage=complete`：source 均有 index entry
- `coverage=stale`：存在 source JSONL 但没有 digest/index entry
- `coverage=partial-current`：缺失的是当前进行中的 session
- `coverage=unknown`：source root 没有可判断的 JSONL

Retrospective / DeepThink 分析会话记录前应先说明 coverage、source count、digest count 和 missing session id。
