...
- 当前任务若使用正式 SDD：当前 SDD 的 `README.md`、`design/INDEX.md`、`tasks.md`、`progress.md`、`bdd.md`，以及设计旁 FeatureSpec（如存在）。
...
- 使用 SDD 或已有设计文档时，DeepThink 只提供分析结论、确认点和默认假设；设计文档写作质量交给 `Workspace/SystemAgent/Rules/DesignDocument.md` / `systemagent-design-document`。设计冻结后的功能实现细节写入 FeatureSpec，用户裁决和采用的默认假设仍写入 `progress.md`。
