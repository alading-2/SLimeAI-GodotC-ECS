...
2. **标准答案分散**：BDD、设计文档、测试 README、artifact 字段之间没有清晰引用关系。
...
这些字段由 TestDesigner 输出，或直接写在设计文档的“行为验收”小节中。SDD `bdd.md` 可以摘录任务需要执行的场景，但长期行为事实源应靠近设计文档。
...
2. 在设计旁写行为验收，必要时同步 SDD `bdd.md` 引用。
