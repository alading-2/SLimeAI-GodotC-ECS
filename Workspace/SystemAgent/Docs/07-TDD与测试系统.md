...
Logger 在测试体系里不只是“打印工具”，而是 **测试与调试共用的证据基础设施**。
...
### Logger 在 TDD / Debug 中的真实职责

- **在 TDD 中**：`ValidationSession` 和 Validation artifact 负责把 RED/GREEN 的判断从“看 stdout”升级成结构化证据。
- **在 Debug 中**：`OperationTrace`、flow conclusion、`logctl analyze` 负责把运行时异常从“分散消息”收口成可定位的失败链路。
- **在 Review / Verify 中**：artifact oracle、structured failure、`no-failure-observed` / `failed` / `passed` 状态语义，决定什么证据可以被接受。

因此，Logger 不只是测试打印层，而是整个 AI-first evidence plane 的一部分；SystemAgent Debug workflow 如何消费这些证据，见 `Workspace/SystemAgent/Docs/10-Debug工作流与证据链.md`。
