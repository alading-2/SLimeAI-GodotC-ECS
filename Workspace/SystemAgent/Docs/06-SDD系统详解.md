...
SDD CLI 的 `new/start/block/task/done/note` 默认只维护 State / Decisions / Validation；旧 `Latest Resume` 和 `P001` timeline 只作为历史 SDD 的读取兼容，不再作为新写入格式。
...
- done 是否仍有未完成任务。
- blocked 是否有 blocker。
- `bdd.md` 是否有场景、Executed features/scenarios、FeatureSpec / Source 引用或 not-required reason。
- 是否存在明显模板残留或空壳完成。
