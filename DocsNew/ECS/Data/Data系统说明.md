...
> 状态：当前实现说明；SDD-0021 no-compat hard cutover 已收口到 DataOS snapshot -> generated typed handle -> catalog-bound Data 主链路。
...
Data 是旧 Godot C# ECS 主线的运行时数据容器。它不再以手写 `DataMeta` / `DataRegistry` 作为字段定义事实源，而是按下面的链路运行：
...
`DataDefinitionCatalog` 是运行时索引。它在启动时注册全部 `DataDefinition`，并建立 computed 反向依赖索引。
...
`new Data()` 会绑定默认 `DataRuntimeBootstrap.Default.Catalog`；需要测试隔离时显式传入 `new Data(catalog)`。运行时 Data 必须始终绑定 `DataDefinitionCatalog`。
...
```csharp
var data = new Data(DataRuntimeBootstrap.Default.Catalog);
